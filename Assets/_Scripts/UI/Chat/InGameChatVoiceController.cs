...
public class InGameChatVoiceController : MonoBehaviour
{
    ...
    private void EnsureUniVoiceReady()
    {
        ...
        try
        {
            ...
            m_voiceClient.OnPeerJoined += HandleVoicePeerJoined;
    ...
    private void HandleVoicePeerJoined(int peerId)
    {
        if (peerId < 0 || m_voiceSession == null)
        {
            return;
        }

        if (!m_voiceSession.PeerOutputs.TryGetValue(peerId, out IAudioOutput output) ||
            output is not StreamedAudioSourceOutput streamedOutput)
        {
            return;
        }

        streamedOutput.gameObject.name = $"UniVoicePeerAudio_{peerId}";

        NetworkPlayerVoiceFollower follower =
            streamedOutput.gameObject.GetComponent<NetworkPlayerVoiceFollower>();
        if (follower == null)
        {
            follower = streamedOutput.gameObject.AddComponent<NetworkPlayerVoiceFollower>();
        }

        follower.Initialize((ulong)peerId);
    }
