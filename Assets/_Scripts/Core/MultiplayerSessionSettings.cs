...
public static class MultiplayerSessionSettings
{
    ...
    private static string s_runtimePlayerName;

    public static string LoadPlayerName()
    {
        if (string.IsNullOrWhiteSpace(s_runtimePlayerName) == false)
        {
            return s_runtimePlayerName;
        }

        string normalized = NormalizePlayerName(PlayerPrefs.GetString(PlayerNamePrefsKey, string.Empty));
        s_runtimePlayerName = string.IsNullOrEmpty(normalized) ? DefaultPlayerName : normalized;
        return s_runtimePlayerName;
    }
    ...
    public static void SavePlayerName(string rawName)
    {
        ...
        s_runtimePlayerName = string.IsNullOrEmpty(normalized) ? DefaultPlayerName : normalized;
        PlayerPrefs.SetString(PlayerNamePrefsKey, normalized);
        PlayerPrefs.Save();
    }
